"""Bridge debug v3: relaxed conditions + 3-way junction recovery."""
import matplotlib
matplotlib.use('Agg')
...
def line_pts(p1, p2):
    """Simple bresenham-like line from p1 to p2."""
...
def do_bridge_relaxed(skel, vm, h, w, dom_mask=None, dom_thresh=0.3,
                      max_dist=100, max_angle=70):
    """Relaxed bridging: wider angle, longer distance, softer dominance."""
    eps = get_eps_tangent(skel, h, w)
    ct = np.cos(np.radians(max_angle)); cands = []
    ...
    for i, j, sc, e1, e2, pts in cands:
        if i not in used and j not in used:
            bridges.append({'ep1': e1, 'ep2': e2, 'pts': pts, 'type': 'pair'})
            for (y, x) in pts:
                if 0<=y<h and 0<=x<w: bridged[y, x] = True
            used.add(i); used.add(j)

    # === 3-way junction recovery ===
    # Find remaining (unused) EPs, cluster nearby ones (< 20px)
    remaining_eps = [eps[i] for i in range(len(eps)) if i not in used]
    if len(remaining_eps) >= 3:
        positions = np.array([e['pos'] for e in remaining_eps])
        dist_mat = cdist(positions, positions)

        visited = set()
        for idx in range(len(remaining_eps)):
            if idx in visited:
                continue
            # Find all EPs within 20px of this one
            nearby = [j for j in range(len(remaining_eps))
                      if j != idx and j not in visited and dist_mat[idx, j] < 20]
            if len(nearby) >= 2:
                # We have 3+ EPs close together -> 3-way junction
                group = [idx] + nearby[:2]  # take up to 3
                group_eps = [remaining_eps[g] for g in group]
                # Centroid
                cy = int(np.mean([e['pos'][0] for e in group_eps]))
                cx = int(np.mean([e['pos'][1] for e in group_eps]))
                # Check centroid is on vessel
                if 0 <= cy < h and 0 <= cx < w and vm[cy, cx]:
                    for g in group:
                        visited.add(g)
                    # Draw lines from each EP to centroid
                    for e in group_eps:
                        pts = line_pts(e['pos'], (cy, cx))
                        for (y, x) in pts:
                            if 0<=y<h and 0<=x<w: bridged[y, x] = True
                    bridges.append({'centroid': (cy, cx), 'eps': group_eps, 'type': 'tri'})
...
def draw_bridges(vis, bridges, color, tri_color, by):
    for br in bridges:
        if br['type'] == 'pair':
            for py, px in br['pts']:
            ...
            if 0 <= cy < by and 0 <= cx < vis.shape[1]:
                cv2.circle(vis, (cx, cy), 4, (0, 255, 0), -1)  # green dot for tri-junction
...
for sid in sids:
    ...
    r_bridged, r_br = do_bridge_relaxed(r_filt, vm, h, w, dom_mask=r_dom)
    b_bridged, b_br = do_bridge_relaxed(b_filt, vm, h, w, dom_mask=b_dom)
    ...
    print(f'{sid}:')
    print(f'  R pre: JN={len(r_jn_pre)} EP={len(r_ep_pre)} | post: JN={len(r_jn_post)} EP={len(r_ep_post)} | pair={r_pair} tri={r_tri}')
    print(f'  B pre: JN={len(b_jn_pre)} EP={len(b_ep_pre)} | post: JN={len(b_jn_post)} EP={len(b_ep_post)} | pair={b_pair} tri={b_tri}')

    # === Figure: 4 rows x 3 cols ===
    fig, axes = plt.subplots(4, 3, figsize=(15, 17))
    fig.suptitle(f'{sid} — Relaxed Bridging + Tri-junction Recovery', fontsize=14, fontweight='bold')
    ...
    vis = np.zeros((by,w,3), dtype=np.uint8)
    vis[cv2.dilate(r_filt[:by].astype(np.uint8), dk2)>0] = [230,70,70]
    draw_bridges(vis, r_br, [255,180,50], [0,255,100], by)
    axes[2,0].imshow(vis); axes[2,0].set_title(f'Bridging: R (pair={r_pair} tri={r_tri})', fontsize=10)

    vis = np.zeros((by,w,3), dtype=np.uint8)
    vis[cv2.dilate(b_filt[:by].astype(np.uint8), dk2)>0] = [70,140,240]
    draw_bridges(vis, b_br, [50,255,180], [0,255,100], by)
    axes[2,1].imshow(vis); axes[2,1].set_title(f'Bridging: B (pair={b_pair} tri={b_tri})', fontsize=10)
    ...
    draw_bridges(vis, b_br, [50,255,180], [0,255,100], by)
    axes[2,2].imshow(vis)
    axes[2,2].set_title(f'Bridging: R+B on GT', fontsize=10)
    ...
    for ax in axes.flat:
        ax.axis('off')
    plt.tight_layout()
    plt.savefig(OUT / f'bridge_v3_{sid}.png', dpi=200, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f'  Saved bridge_v3_{sid}.png')
